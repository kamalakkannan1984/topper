...
public function getMsgFilename($r){
			$promoCode 		= $r->promoCode;
		$sql = "SELECT FileName FROM Message WHERE NOW() between StartDate and EndDate AND CodeName ='$promoCode'";
...
public function checkInstallCashback($r){
	...
	$IMEI_2 = $r->IMEI;
	$res = array();
	$stmt = $this->conn->prepare("SELECT PromoName from TopUserInfo WHERE IMEI_1 = ? OR IMEI_2 = ?");
